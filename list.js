  ...
  var files = $.map(xml.find('Contents'), function(item) {
    ...
    return {
      Key: item.find('Key').text(),
          LastModified: parseXSDDateString(item.find('LastModified').text()),
...
function prepareTable(info) {
  ...
  var cols = [18, 12, 45];
  var content = [];
  content.push(padRight('Last Modified', cols[0]) + '  ' +
               padRight('Size', cols[1]) + 'Key \n');
...
function renderRow(item, cols) {
  var row = '';
  row += padRight(item.LastModified, cols[0]) + '  ';
  row += padRight(item.Size, cols[1]);
